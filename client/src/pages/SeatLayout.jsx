...
const SeatLayout = () => {
  ...
  const handleSeatClick = (seatId) => {
    ...
    if (occupiedSeats.includes(seatId)) {
      return toast('Seat already occupied')
    }
    ...
    <div key={row} className='flex gap-2 mt-2'>
      <div className='flex flex-wrap items-center justify-center gap-2'>
        {Array.from({ length: count }, (_, i) => {
          ...
          return (
            <button key={seatId} onClick={() => handleSeatClick(seatId)} className={`h-8 w-8 rounded border border-primary/60 cursor-pointer hover:text-primary/90 transition-colors ease-in-out ${selectedSeats.includes(seatId) && "bg-primary text-white"} ${occupiedSeats.includes(seatId) && "opacity-50"}`}>
              {seatId}
            </button>
          )
        })}
        ...
        <div className='mt-5 space-y-1'>
          {show.dateTime[date].map((item) => (
            ...
            >
              ...
              <p className='text-sm'>
                {isoTimeFormat(item.showDateTime)}
              </p>
            </div>
          ))}
